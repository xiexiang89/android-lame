...
public class DataEncodeThread extends HandlerThread implements AudioRecord.OnRecordPositionUpdateListener {
	...
	private static final int MSG_RECORD_FINISH = 0x1;
		...
		public void handleMessage(Message msg) {
			...
			switch (msg.what){
				case MSG_RECORD_FINISH:
					mMp3Recorder.handleRecorderFinish();
					break;
			}
		}
	};
	...
	private static class StopHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			if (msg.what == PROCESS_STOP) {
				...
				getLooper().quit();
				encodeThread.sendRecordFinishMessage();
			}
		}
	}

	private void sendRecordFinishMessage(){
		mMainHandler.obtainMessage(MSG_RECORD_FINISH).sendToTarget();
	}
	...
	private void flushAndRelease() {
		//将MP3结尾信息写入buffer中
		final int flushResult = LameUtils.flush(mMp3Buffer);
		if (flushResult > 0) {
			try {
